...
export default {
    ...
    methods: {
        generateLoading() {
            ...
            let y = 100
            setInterval(() => {
            this.intervalTime(cxts, x, y, angles, radius)
            angles++
            if (angles === 360) {
                angles = 0
                this.clearLoading(cxts, x, y, radius)
            }
            }, 1000/360)
        },
        intervalTime(cxts, x, y, angles, radius) {
            if(!this.gecLoading) return
            cxts.beginPath()
            cxts.lineWidth=3
        ...
        clearLoading(cxts, x, y, radius) {
            cxts.clearRect(x-radius, y-radius, 2*radius, 2*radius)
        },
    }
}
</script>
